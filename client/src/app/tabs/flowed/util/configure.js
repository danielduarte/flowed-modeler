...
export default function configureModeler(
  ...
  const dynamicMiddlewares = getPlugins('flowed.modeler.configure').map(fn => {
    ...
    return function wrappedMiddleware(options, logWarning) {
      ...
      try {
        ...
        if (!newOptions) {
          logWarning('flowed.modeler.configure does not return options');
        }
  ...
  const middlewares = [
    ...
    function moddleExtensionsMiddleware(options, logWarning) {
      const plugins = getPlugins('flowed.modeler.moddleExtension');
      ...
      const moddleExtensions = plugins.reduce((extensions, extension) => {
        ...
        if (typeof name !== 'string') {
          logWarning('flowed.modeler.moddleExtension is missing <name> property');
        ...
        if (name in extensions) {
          logWarning('flowed.modeler.moddleExtension overrides moddle extension with name <' + name + '>');
        }
    ...
    function additionalModulesMiddleware(options) {
      const additionalModules = getPlugins('flowed.modeler.additionalModules');
