...
import FlowedModeler from './modeler';

import { active as isInputActive } from '../../../util/dom/isInput';

import getFlowedContextMenu from './getFlowedContextMenu';

import { getFlowedEditMenu } from './getFlowedEditMenu';

import getFlowedWindowMenu from './getFlowedWindowMenu';

import css from './FlowedEditor.less';
...
export class FlowedEditor extends CachedComponent {
  ...
  handleChanged = () => {
    ...
    const contextMenu = getFlowedContextMenu(newState);

    const editMenu = getFlowedEditMenu(newState);

    const windowMenu = getFlowedWindowMenu(newState);
  ...
  render() {
    ...
    return (
      <div className={ css.FlowedEditor }>
  ...
  static createCachedState(props) {
    ...
    if (warnings.length && isFunction(onError)) {
      onError(
        'Problem(s) configuring Flowed editor: \n\t' +
        warnings.map(error => error.message).join('\n\t') +
        '\n'
      );
    }

    const modeler = new FlowedModeler({
      ...options,
      position: 'absolute'
    });
...
export default WithCache(WithCachedState(FlowedEditor));
